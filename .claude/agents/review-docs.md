...
6. **OpenSpec change hygiene** (only when the diff touches `openspec/changes/<change>/`)
   - An active change should carry a `README.md` front door and a deliberation
     record — a real `deliberations/NNN-slug/` run OR `deliberations/000-skipped.md`.
     Flag a change missing either.
   - The change README's `<!--STATUS:LINE-->` / `<!--STATUS:AUTO-->` blocks are
     generated by `bin/readme-sync` — flag hand-edits inside the markers, and flag a
     status block that looks stale vs the `tasks.md` checkboxes.
   - Memory is **event-driven**: do NOT flag the absence of per-task `session-log.md`
     entries or a "Current Status" section — `tasks.md` checkboxes are the per-task
     record. Only flag the reverse (per-task prose churn) if it's clearly noise.
