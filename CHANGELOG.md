...
- **The restart prompt is now contextual — no more permanent "Apply & restart" button.** Player feedback: a button that sat there (mostly disabled) implied everything was staged-then-applied, when in fact almost everything applies live. The panel now applies every change it can **instantly** (effects, volume, accessibility, and shadows on a capable tier), and a **"Restart now" bar slides in at the bottom only when a change genuinely needs a reload** — a quality-tier switch, the extra-light toggles, or the lone shadows edge (forcing it On on the low tier). Change nothing reload-worthy and the bar never appears. Backing this, shadows were simplified so Off/Auto/On are all live on mid/high (the boot flag now only force-enables the shadow map for a low-tier pin). ([settings.js](src/settings.js), [perf.js](src/perf.js), [styles.css](styles.css))
...
### Fixed
- **Shadows can now be pinned against the adaptive governor — "On" actually stays on.** The Settings "Shadows" control was only a boot flag, so the runtime quality governor was still free to auto-drop shadows under load (its ladder has a "no-shadows" rung) — set Shadows on, drive into something heavy, and they'd vanish. Shadows is now a proper **Off / Auto / On** tri-state in Effects (alongside Glow and Detailed bubbles), and the governor honors the pin (`AdaptiveQuality.effectiveShadows`): "On" keeps them no matter how far the governor ramps down; "Auto" lets it manage them; "Off" turns them off. The pin is **live** on a tier that already has shadows — Off, Auto, and On all apply with no reload. The one exception that needs a restart is the genuine edge: forcing shadows **On on the low tier**, where the shadow map was never built at boot. ([adaptiveQuality.js](src/adaptiveQuality.js), [perf.js](src/perf.js), [settings.js](src/settings.js))
