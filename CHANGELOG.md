...
### Performance
- **Star power is free at idle.** No added draw calls when no star is placed and no buff is running — every wave/trail/shockwave mesh sits `visible=false` and the rainbow shader branch is gated behind `uStarEnv > 0.001`, so steady-state cost is zero geometry and zero per-frame allocation. A placed star is +2 draws (star + pillar, plus one `contextLights`-gated PointLight on mid/high). During the rare 15 s buff the additive ring effects add ~11 transient draws on mid/high — and are deliberately gated down to **2 on low tier** (one love-wave ring, no comet trail) to protect the 80-draw low budget.
