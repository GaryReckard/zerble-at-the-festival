...
### Fixed
- **Shadows can now be pinned against the adaptive governor — "On" actually stays on.** The Settings "Shadows" control was only a boot flag, so the runtime quality governor was still free to auto-drop shadows under load (its ladder has a "no-shadows" rung) — set Shadows on, drive into something heavy, and they'd vanish. Shadows is now a proper **Off / Auto / On** tri-state (in "Shadows & lights"), and the governor honors the pin (`AdaptiveQuality.effectiveShadows`): "On" keeps them no matter how far the governor ramps down; "Auto" lets it manage them; "Off" turns them off. Shadows is **reload-to-apply** — the sun light's `castShadow`, the shadow map, and every material's shadow sampling are wired at boot from `PERF.shadows` and can't be reconstructed live — so a change shows the restart bar; on the next boot the choice applies and the governor picks up the pin, so once on it stays on. ([adaptiveQuality.js](src/adaptiveQuality.js), [perf.js](src/perf.js), [world.js](src/world.js), [settings.js](src/settings.js))
