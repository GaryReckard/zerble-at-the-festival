...
- **Food courts stop stacking a second bubble vendor on top of the hub's (worldgen v2, flag-off).** Every hub gets one guaranteed bubble vendor (D15), and a food court added another ~40% of the time with **no nearby-vendor check**, so two identical booths could land cheek-to-cheek (Gary 2026-06-16, seed 1466805657: "TWO bubble booths, shouldnt find them so close"). The food-court booth now skips when a `bubble_vendor` is already within 22 m. Build-time/registry dedup — the rng rolls still fire, so the chunk draw stream and both goldens are unchanged.
