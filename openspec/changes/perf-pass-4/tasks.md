...
- [x] 3.1 Audit which `buildWorldgenKind` cases register colliders; pin ALL collider-registering work to the synchronous structure phase. *(Nine of ten do: main/side/tent stages, arch, food court, vendor row, bubble vendor, porta bank, and drum circle. `camp_village` is the only wholly collider-free case. The drum access path is collider-free but shares a case with its collidable circle. Audit is codified beside the dispatcher.)*
- [x] 3.2 Switch the ChunkManager budget from `BUDGET_PER_FRAME` chunks to a tier-aware `chunkBudgetMs` wall (3/4/5ms low/mid/high); keep `firstLoad` eager. *(The loop starts the closest missing chunk, then starts another only if the elapsed generation wall remains. Real Start boot passed at low/mid/high with eager 9/25/25-chunk rings; a high-tier 800m teleport streamed the next 25 chunks while frames advanced and the page-error log stayed empty.)*
- [ ] 3.3 **RE-SCOPED by D24:** do not build the old whole-stage deferred queue. The debug stage profiler proved that safe whole-stage work is only ~6ms across nine low-tier travel chunks, while structural props + collidable-tree planning cost ~230ms. A useful C1-b must first split hot structural stages into synchronous placement/collider planning plus deferred visual materialization without changing registry-read or rng order; tree instanced-mesh materialization itself is only ~0.4ms, so begin with vendor-row descriptor/build separation or a measured tree-planning optimization. Crowd injection remains last after any deferred visuals.
...
- [x] 5.2a Extract `mergeStaticDecor` + `_bakeForMerge` + `_MERGED_*` mats from tent.js → neutral `src/mergeDecor.js` (preserve the `userData.shared` mat tags, tent.js:86/88); update tent.js + picnicTable.js call sites; add `mergeDecor` to ALL FOUR importmaps; run `bin/check-importmaps`. *(Current `picnicTable.js` already self-merges independently and had no tent import to repoint.)*
- [x] 5.2b Harden the util: add the missing `if (o.isInstancedMesh) return;` guard (Critical — camp-style instanced fields would frozen-collapse); keep emissive + `noMerge` skips; surface `castShadow` per bucket (currently hardcoded true for opaque, tent.js:148); keep the failure-safe ordering (merge fails → originals stay).
- [x] 5.2c Apply per-model merge inside `buildFoodTruck` + `buildSugarShack` only; tag the cook/animated/emissive parts `noMerge`; honor the disposal contract (merged geo NOT shared → disposes on unload; merged mats `userData.shared`; originals disposed only behind `!userData.shared`). rng-free (merge call is last in the builder).
- [x] 5.2d **Gary's real-GPU capture (the gate):** draws before/after parked at a dense food-court on `?perf=low/mid/high`; `renderer.info.memory.geometries` returns to baseline across a hub load/unload cycle (leak check); **Midnight** screenshot (emissive glow intact); two-frame check (animated parts still move). **Result:** broad food-truck + Sugar Shack merges scoped OUT after draws rose +72/+358/+364 on low/mid/high and the merged lifecycle stopped advancing after court-3; production defaults unmerged. The proven window/sign-shell cuts remain. Final Midnight route verified with intact food-court lighting and advancing frames.
- [x] 5.2e **Permanent model-level measurement surface:** add a post-`RenderPass` scene-info capture to `sandbox.html` and show scene draws/tris plus renderer geometry/texture counts in the entity panel. Make entity teardown honor `userData.shared` and dispose InstancedMesh instance buffers, then prove a food-truck → Sugar Shack → food-truck cycle reaches a stable resource plateau with no console errors.
- [x] 5.2f **Measured follow-up cut:** replace each Sugar Shack sign's six-material box with one mergeable five-face shared shell plus one texture-backed front plane (72 → 52 scene draws, triangles unchanged at 6,431); premerge the food truck's two emissive windows into one shared geometry (7 → 6 scene draws, triangles unchanged at 344). Verify both models at Noon + Midnight and boot the full game on forced low/mid/high.
- [x] 5.3 **Fog-as-far-cull shipped:** `PERF.cameraFar=1040` on all tiers, the shortest backdrop-safe range (fog ends 520m; sky 900m; stars 850m; worst-case mountains ~1012m; ground corners ~990m). Fixed-registry travel A/B/A measured 2595–2610 draws vs 2949–2960 at the old 1500m, roughly 350 fog-hidden retained-lake draws and 12k tris removed. Same-state Noon control/shipping frames were visually indistinguishable; Midnight sky/stars/mountains intact; real Start boot clean on low/mid/high.
