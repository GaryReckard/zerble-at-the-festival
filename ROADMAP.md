...
- **Bubble-juice meter follow-ups.** The meter shipped (see CHANGELOG 2026-06-01) — drains while bubbling, ~3× on the G blast, never-dry trickle, refueled by rare floating jugs + the spacesuit bubble vendor, top-left HUD gauge. Parked refinements: (1) the **"costs smiles" economy** — vendor refills currently free; a smile cost (full or token) is the obvious sink for the score currency if the loop needs more stakes. (2) **Bubble-variety juice costs** — once bubble varieties ship, mega-bubbles could cost more juice and glow bubbles less. (3) **Drain tuning by feel** — if it nags on a real playtest, drop the drain or gate it behind an opt-in "survival" mode. (4) A **low-juice audio cue** (sputter sound) to match the visual amber warning.
