...
- **Bubble-juice meter — a resource layer on the core verb.** Right now bubbling is free and infinite; a juice tank adds a gentle spend/replenish loop. A `bubbleJuice` 0..1 reserve drains slowly while bubbling and **~3× faster while the `G` blast is held** (`bubbles.js` `setBlast` is the obvious drain multiplier — it already cranks output ~2.8×, so the cost scales with the payoff). Small HUD gauge near the smile counter (DOM, reuse the `hud.js` score-panel patterns — a vertical "fill tube" reads on-theme).

  **Protect the zen.** The whole pitch is "bring the bubbles, collect the smiles" — a meter that strands you bubble-less would fight that. So: generous tank, slow baseline drain, and **never fully empties** — at zero the spawner throttles to a thin trickle (smaller, sparser bubbles) rather than cutting off. It's a throttle, not a wall. Blasting on empty just sputters.

  **Refuel — the interesting part. Don't lean on jugs.** Plain spawned jugs work but are the boring answer. Better options, ideally combined:
  - **Smiles top you off (recommended primary).** Every smile you collect trickles juice back (`smiles.js` pickup callback in `main.js` already increments score — same hook). This closes the loop: bubble → smile → more juice → bubble. Play well and you're self-sustaining; idle-blast into empty air and you run down. The replenish *is* the core gameplay, so there's no pit-stop tax — it rewards exactly what the game's about.
  - **Water refills it (recommended exploration source).** Bubble juice is soapy water — cruise a shoreline (or through a fountain/sprinkler) and the tank refills fast. We **already compute "lakeness"** every frame for the frog ambience ([main.js](src/main.js)), so a shoreline top-up is nearly free to wire. Bonus: it finally gives lakes a *mechanical* reason to exist beyond canoes + scenery, and rewards world exploration.
  - **Bubble-juice fountain / vendor cart (flavor).** A festival art-installation bubble fountain you park beside (glug-glug refill), or a "bubble juice" stand that slots straight into the **Vendor stand power-ups** idea below (reuse `foodTruck.js` builders + the food-truck attractor trigger).
  - **Floating juice pickups (flavor).** Occasional floating jug/orb, grabbed like a smile (reuse the smile-pickup rise+homing in `smiles.js`).
  - **Cheeky tie-ins.** A wook trip = overflowing juice (everything's abundant, man — gate on `Trip._envelope`); catching Lurleen tops the tank to full (love is fuel).

  **Recommended shape:** smiles trickle it back (self-sustaining core loop) + shorelines/fountains for a fast top-up (exploration), generous + never-dry. Jugs/pickups optional sprinkle.

  **Tension to weigh:** this adds friction to the *signature* verb — the risk is it nags more than it engages. The never-dry trickle + smile-refuel are what keep it a fun rhythm rather than a chore; if playtesting feels naggy, dial the drain down hard or make it an opt-in "survival" mode rather than the default. Pairs naturally with the bubble-varieties idea above (e.g. mega-bubbles cost more juice; glow bubbles are cheaper).

  Implementation sketch: `bubbles.js` owns `juice` + `drainRate`, decremented in `update()` (× blast multiplier), and gates emit count / size on it; `main.js` adds juice on smile pickup and on the per-frame `lakeness` proximity already computed; a `#juice` DOM gauge in `hud.js`; persist nothing (fresh tank each session, like score resets are session-scoped). No new geometry for the core mechanic.
...
- **Forest tree count on low tier.** PERF tier currently scales chunk draw radius and crowd density but not forest tree density. Dense forests on mid-spec phones might benefit from a tier-gated thin-out. *More relevant since trees went 2x (2026-06-01)* — bigger canopies mean more overdraw/fill, so a low-tier density trim (cut `FOREST_TREE_TARGET_DENSITY` ~25–30%, the bigger trees fill the gaps) would offset the fill without thinning the look much. Deferred pending a real-device feel-test rather than shipped speculatively.
