...
- **Geometry merging / instancing follow-ups.** P1 shipped 2026-06-16: vendor booths collapse static decor to two vertex-color meshes (−36% meshes on a dense hub). The safe half of P2 landed 2026-07-15: food trucks + Sugar Shacks merge per model through `src/mergeDecor.js`, with the remaining multi-material signs and co-material truck windows reduced after model-level measurement; camp villages were deliberately skipped because their repeated props are already pooled/instanced and their animatable references make a blind merge high-risk. Remaining: the real-GPU Task 5.2d dense-cluster/lifecycle gate, then **P3** — per-row instancing of shopkeeper NPCs + transparent jars and LOD on distant clusters, since a dense forward view is still ~3–4k meshes in frustum.
